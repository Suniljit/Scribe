...
def _run_dual_track_pipeline(
    ...
    mic_segments, speaker_segments = _dedup_bleed_segments(mic_segments, speaker_segments)
...
def _dedup_bleed_segments(
    ...
    speaker audio acoustically leaked into the mic (ADR 0005). Always runs
    on dual-track recordings - not gated on `Recorder`'s acoustic
    `_detect_bleed` signal, which in practice under-detects real bleed (see
    ADR 0005's "always match on text, don't gate on acoustic detection"
    note) - and relies entirely on the text-similarity threshold below to
    avoid suppressing genuine same-text same-time utterances on clean
    dual-track recordings.

    Matching is many-to-one aware: whisper can segment the same stretch of
    speech very differently per track (e.g. one track's VAD merges several
    sentences - and speaker turns - into a single run-on segment, while the
    other splits them normally). Comparing a short segment's whole-string
    similarity against a much longer one would score low purely from length
    mismatch even when the short text is fully contained in the long one, so
    `_contains_ratio` scores containment instead. Matched segments are
    grouped (via union-find on the mic/speaker overlap graph) and the
    coarser side of each group - the one with fewer segments - is dropped,
    since collapsing multiple segments into one loses information (like
    per-speaker turn boundaries) a 1:1 avg_logprob tie-break can't recover.
    A genuine 1:1 match still falls back to that avg_logprob comparison.
    """
    ...
    def similarity(a: TranscriptSegment, b: TranscriptSegment) -> float:
        ...
        if len(a_text) <= len(b_text):
            return contains_ratio(a_text, b_text)
        return contains_ratio(b_text, a_text)

    n = len(mic_segments)
    parent = list(range(n + len(speaker_segments)))
    ...
    for i, mic_seg in enumerate(mic_segments):
        for j, speaker_seg in enumerate(speaker_segments):
            if not overlaps(mic_seg, speaker_seg):
                continue
            if similarity(mic_seg, speaker_seg) > BLEED_DEDUP_SIMILARITY_THRESHOLD:
                edges.append((i, j))
                union(i, n + j)
    ...
    for i, j in edges:
        mic_idxs, speaker_idxs = groups.setdefault(find(i), (set(), set()))
        mic_idxs.add(i)
        speaker_idxs.add(j)
    ...
    for mic_idxs, speaker_idxs in groups.values():
        if len(mic_idxs) > len(speaker_idxs):
            drop_speaker |= speaker_idxs
        elif len(speaker_idxs) > len(mic_idxs):
            drop_mic |= mic_idxs
        else:
            for i, j in edges:
                if i not in mic_idxs or j not in speaker_idxs:
                    continue
                mic_seg, speaker_seg = mic_segments[i], speaker_segments[j]
                if (
                    mic_seg.avg_logprob is not None
                    and speaker_seg.avg_logprob is not None
                ):
